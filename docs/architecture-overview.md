...
共有ライブラリは `hooks/lib/`（出力スキーマ＝`emit.sh`、破壊パターン＝`patterns.sh`、入力抽出＝`extract-input.sh`、frontmatter ＝`frontmatter.sh`、観測ログ＝`evidence.sh`/`fingerprint.sh`、テスト実行 positive proof ＝`marker.sh`、phase→skill マップ＝`phase-skills.sh`、credential パターン＝`secrets-patterns.sh`、fail-closed integrity ＝`safety.sh`、untrusted 文中和＝`sanitize.sh`、layer-2 OS lock ＝`cp-lock.sh`、gate snapshot ＝`snapshot.sh` の計 13 本）。
