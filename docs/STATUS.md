...
task_type: framework
task_size: M
task_size_rationale: "iteration 53（framework・破壊的コマンド警告の日本語化＋ドリフトガード）= M。footprint は hooks/lib/patterns.sh（WARN 配列18件）＋hooks/check-destructive.sh（inline rm -r＋英語フォールバック）＋hooks/check-secrets.sh（英語フォールバック）＋tests/test_destructive_warning_language.py（新規）＝4 ファイル。判定ロジック（正規表現・ask 発火）無改変＝moat 不変。必須ゲート review+qa+security（deploy は M で size-exempt）。"
iteration: 53
ui_surface: false
last_updated: "2026-06-29T02:15:00Z"
gate_approvals:
...
current_refs:
  requirements: []
  plan: docs/plans/2026-06-28-destructive-warning-japanese-implementation-plan.md
  spec: docs/specs/2026-06-28-destructive-warning-japanese-design.md
  review: docs/qa-reports/iter53-review.md
  qa: docs/qa-reports/iter53-qa.md
  security: docs/qa-reports/iter53-security.md
  deploy: null
  translation: null
external_evidence:
