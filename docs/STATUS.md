...
task_type: framework
task_size: M
task_size_rationale: "iteration 45 = full-review 残 C2+C3（bin/setup.sh）。C2: 引数パーサを while+shift 2 アーム化し --profile/--target を = と空白の両形式対応（doc/impl 不整合解消・North Star）。C3: FRAMEWORK_VERSION heredoc を argv 渡しに変え dead first path 解消（機能影響ゼロの cleanup・grep フォールバック存置）。size=M（production 1=bin/setup.sh ＋ test 1=新規 tests/test_setup_arg_version.py の 2 files）。type=framework（setup.sh は moat 配布の要＝security 必須）。M は deploy 自動 exempt。設計: docs/specs/2026-06-25-iter45-setup-arg-version-design.md。**やらない**: エラー親切化のみ案・heredoc 削除案・C4/G4・check-control-plane 再設計。"
iteration: 45
ui_surface: false
...
current_refs:
  ...
  plan: docs/plans/2026-06-25-iter45-setup-arg-version-implementation-plan.md
  spec: docs/specs/2026-06-25-iter45-setup-arg-version-design.md
  review: docs/qa-reports/iter45-review.md
  qa: docs/qa-reports/iter45-qa.md
  security: docs/qa-reports/iter45-security.md
  deploy: null
  translation: null
external_evidence:
