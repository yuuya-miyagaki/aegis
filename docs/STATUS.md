...
task_type: framework
task_size: M
task_size_rationale: "iteration 49（framework・配布 self-containment 射程拡大）= **M**（2-5 files）。grill-plan で premise 訂正: 旧 3 穴のうち #1(validate)/#2(retro) は false positive（setup.sh:resolve_source が templates/commands/ の scaffold-safe 版を install＝graceful degrade）。実穴は #3 skill→update-task.sh の 1 件。確定 slice: (1)update-task.sh を standard+full の required に同梱（required な update-gate.sh の sibling・proven inert ref: skills[full]/state-machine.md[standard]） (2)iter48 test に skill→script 参照検査を追加（skills は verbatim install＝唯一 scaffold-safe 保護外） (3)README standard 件数同期。command→script・validate de-ship・retro_report 同梱は破棄（YAGNI/既解決）。変更: test 1・profile JSON 2・README 1＝4 files。必須ゲート review+qa+security（deploy は M で size-exempt）。"
iteration: 49
ui_surface: false
last_updated: "2026-06-27T03:00:00Z"
gate_approvals:
...
current_refs:
  ...
  spec: docs/specs/2026-06-27-command-skill-ref-integrity-design.md
  review: docs/qa-reports/iter49-review.md
  qa: docs/qa-reports/iter49-qa.md
  security: docs/qa-reports/iter49-security.md
  deploy: null
  translation: null
external_evidence:
