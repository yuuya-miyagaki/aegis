...
task_type: framework
task_size: M
task_size_rationale: "iteration 51（framework・確認(permission prompt)交通整理 第一スライス＝安全な読み取り/記録系コマンドの permissions.allow 同梱）= M。footprint は templates/hooks.template.json＋bin/setup.sh:generate_settings()＋tests の複数ファイル（filtered branch で permissions を carry／既存ユーザ allow との union／全プロファイル install e2e／update-gate.sh が allow に入らない negative assert／moat 健在）。必須ゲート review+qa+security（deploy は M で size-exempt）。"
iteration: 51
ui_surface: false
last_updated: "2026-06-28T07:30:00Z"
gate_approvals:
...
current_refs:
  requirements: []
  plan: docs/plans/2026-06-28-permission-prompt-allowlist-implementation-plan.md
  spec: docs/specs/2026-06-28-permission-prompt-allowlist-design.md
  review: docs/qa-reports/iter51-review.md
  qa: docs/qa-reports/iter51-qa.md
  security: docs/qa-reports/iter51-security.md
  deploy: null
  translation: null
external_evidence:
