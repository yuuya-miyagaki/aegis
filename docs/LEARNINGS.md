...
- [confidence:8] `run_hook`（test_hook_output_schema 等）が `os.environ.copy()` で **`CLAUDE_PROJECT_DIR` を継承**するため、scratch STATUS を書いても hook が**実リポの STATUS を読む**ことがある＝判定が実リポの現在状態（task_size 等）に依存し、「実 size=L のときだけ deny で運頼みに pass」する潜在分離バグになる（iteration 36・deploy-gate テストで発覚）。hook テストは `CLAUDE_PROJECT_DIR` を scratch に明示固定し、scratch STATUS に判定条件を完全に書くこと。
