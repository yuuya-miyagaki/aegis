...
- **封鎖範囲（iter75 実測封鎖）**: quote(`"`/`'`)・backslash・backslash-newline・`${IFS}`/`$IFS`・broad-stage 難読化(`git${IFS}add -A`)・commit 難読化(`git${IFS}commit`)・難読化大文字クラス全体（rm/chmod/find/dd/shred/mkfs/git 系＝全 `AEGIS_DESTRUCTIVE_CMD_REGEX`・`grep -i`）。回帰 pin＝`tests/test_moat_quote_split.py`（36 ケース）＋`tests/test_patterns_parity.py`。
- **状態**: **実装済み（iter75・commit 5398e72..9c3d7ea・review 再々走中）**。正本＝`docs/full-review-2026-07-19-dual-codex-fable.md` §4.1・実装計画＝`docs/plans/2026-07-20-iter75-moat-fix-forward-plan.md`。**review 盲検2次が2回にわたり残穴を摘発（1回目: broad-stage/commit 難読化・backslash-newline→fix-forward／2回目: 難読化大文字が非rm 全 CMD_REGEX で回避→FF7 で grep -i 化）＝独立レビューが1次の盲点を段階的に捕捉**。CLOSED-in-review は review+security ゲート approve 後。
