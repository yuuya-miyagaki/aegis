#!/usr/bin/env python3
"""K-5 (v1.6.2): 6 deny hook の safety.sh fallback ブロックが byte-identical
であることを契約化する（grill 致命 5）。

safety.sh が source できないという最悪ケースで明示 DENY を出すための
inline fallback は、6 hook で SHA256 一致が義務。drift が起きると JSON
スキーマや reason 文字列が分岐し、将来の Claude Code 仕様変更（例:
hookEventName → hook_event_name）で 6 箇所同期漏れが起きる。
...
class TestSafetyFallbackIdentity(unittest.TestCase):
    ...
    def test_all_deny_hooks_have_identical_fallback(self):
        ...
        self.assertEqual(
            len(digests), 1,
            f"safety fallback block drift across {DENY_HOOKS}: {digests}"
        )
    ...
    def test_fallback_has_no_dynamic_substitution(self):
        ...
        block = _extract_fallback(ROOT / "hooks" / DENY_HOOKS[0])
